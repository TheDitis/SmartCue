...
class Box:
    ...
    def df(self, new_df: pd.DataFrame):
        if isinstance(new_df, pd.DataFrame) and self._validate(new_df):
            self._df = new_df
        else:
            raise ValueError("Tried to set the root df of Box to invalid obj")
    ...
    def midway_box(self, other_box: 'Box') -> 'Box':
        new_box = pd.concat([self.df, other_box.df]) \
            .groupby("loc", as_index=False) \
            .agg({
                'x': np.mean, 'y': np.mean, 'loc': min,
                'h_loc': min, 'v_loc': min
            })
        new_box.index = new_box["loc"]
        return new_box.Box
